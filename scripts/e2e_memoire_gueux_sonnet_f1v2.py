"""JETABLE — Génération mémoire SONNET-F1v2 (Sonnet a/b + Opus méthodologie).
...
Sortie : docs/comparaison-memoire-AB/memoire-gueux-SONNET-F1v2.{md,json}.
"""
...
async def main():
    ...
    print("=" * 70)
    print(f"GÉNÉRATION SONNET F1v2 — Gueux lot 02 — mapping={_MEMOIRE_SEGMENT_MODELS}")
    print(">>> 1 GÉNÉRATION API (4 appels : 4 Sonnet, prompt F1 (EPI corpus-only)) <<<")
    print("=" * 70)
    t0 = time.time()
    ...
    (OUT / "memoire-gueux-SONNET-F1v2.json").write_text(
        json.dumps(content, ensure_ascii=False, indent=2), encoding="utf-8")
    md = render_markdown(content)
    (OUT / "memoire-gueux-SONNET-F1v2.md").write_text(md, encoding="utf-8")
    ...
    print(f"\nRÉSULTAT SONNET-F1v2 : {present}/{total_expected} sous-sections — manquantes: {missing or 'AUCUNE'}")
    ...
    print(f"COÛT TOTAL SONNET-F1v2 estimé : ≈ {total_cost:.3f} $")
