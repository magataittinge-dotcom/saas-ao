"""JETABLE — Génération mémoire SONNET-F1 (Sonnet a/b + Opus méthodologie).
...
Sortie : docs/comparaison-memoire-AB/memoire-gueux-SONNET-F1.{md,json}.
"""
...
async def main():
    ...
    print("=" * 70)
    print(f"GÉNÉRATION SONNET F1 — Gueux lot 02 — mapping={_MEMOIRE_SEGMENT_MODELS}")
    print(">>> 1 GÉNÉRATION API (4 appels : 4 Sonnet, prompt F1 (few-shot+quota)) <<<")
    print("=" * 70)
    t0 = time.time()
    ...
    (OUT / "memoire-gueux-SONNET-F1.json").write_text(
        json.dumps(content, ensure_ascii=False, indent=2), encoding="utf-8")
    md = render_markdown(content)
    (OUT / "memoire-gueux-SONNET-F1.md").write_text(md, encoding="utf-8")
    ...
    print(f"\nRÉSULTAT SONNET-F1 : {present}/{total_expected} sous-sections — manquantes: {missing or 'AUCUNE'}")
    ...
    print(f"COÛT TOTAL SONNET-F1 estimé : ≈ {total_cost:.3f} $")
