"""JETABLE — Génération mémoire SONNET-RENFORCE (Sonnet a/b + Opus méthodologie).
...
Sortie : docs/comparaison-memoire-AB/memoire-gueux-SONNET-RENFORCE.{md,json}.
"""
...
async def main():
    ...
    print("=" * 70)
    print(f"GÉNÉRATION SONNET RENFORCÉ — Gueux lot 02 — mapping={_MEMOIRE_SEGMENT_MODELS}")
    print(">>> 1 GÉNÉRATION API (4 appels : 4 Sonnet, prompt densité renforcé) <<<")
    print("=" * 70)
    t0 = time.time()
    ...
    (OUT / "memoire-gueux-SONNET-RENFORCE.json").write_text(
        json.dumps(content, ensure_ascii=False, indent=2), encoding="utf-8")
    md = render_markdown(content)
    (OUT / "memoire-gueux-SONNET-RENFORCE.md").write_text(md, encoding="utf-8")
    ...
    print(f"\nRÉSULTAT SONNET-RENFORCE : {present}/{total_expected} sous-sections — manquantes: {missing or 'AUCUNE'}")
    ...
    print(f"COÛT TOTAL SONNET-RENFORCE estimé : ≈ {total_cost:.3f} $")
