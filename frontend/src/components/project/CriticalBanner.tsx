...
function Tile({ icon, label, children, accent }: {
  ...
  return (
    <div className="flex items-start gap-2">
      <span className="shrink-0 mt-0.5" style={{ color: accent ?? '#64748B' }}>{icon}</span>
      <div>
        <p className="text-xs font-bold uppercase tracking-wide" style={{ color: '#64748B' }}>{label}</p>
        <div className="text-sm" style={{ color: accent ?? '#334155' }}>{children}</div>
      </div>
    </div>
  )
}
...
export default function CriticalBanner({ projectId, onOpenSource }: Props) {
  ...
  return (
    <div className="rounded-lg px-5 py-3 space-y-3" style={{ background: '#F8FAFC', border: '1px solid #F1F5F9' }}>
      {tiles.length > 0 && (
      ...
      {criteres && totalPoids > 0 && (
        <div>
          ...
          <div className="flex h-2.5 rounded-full overflow-hidden" style={{ background: '#E2E8F0' }}>
            {criteres.map((c, i) => (
              <div
                key={i}
                title={`${c.nom} — ${c.poids} %`}
                style={{
                  ...
                  background: /prix/i.test(c.nom) ? '#0F172A' : i % 2 === 0 ? '#0EA5E9' : '#38BDF8',
                }}
              />
            ))}
          </div>
          <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1">
            {criteres.map((c, i) => (
              <span key={i} className="text-[11px]" style={{ color: '#64748B' }}>
                <span className="inline-block w-2 h-2 rounded-full mr-1 align-middle"
                  style={{ background: /prix/i.test(c.nom) ? '#0F172A' : i % 2 === 0 ? '#0EA5E9' : '#38BDF8' }} />
