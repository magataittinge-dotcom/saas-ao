...
function Tile({ icon, label, children, accent }: {
  ...
  return (
    <div className="flex items-start gap-2">
      <span className="shrink-0 mt-0.5" style={{ color: accent ?? '#9BA4B5' }}>{icon}</span>
      <div>
        <p className="text-xs font-bold uppercase tracking-wide" style={{ color: '#9BA4B5' }}>{label}</p>
        <div className="text-sm" style={{ color: accent ?? '#E8EBF2' }}>{children}</div>
      </div>
    </div>
  )
}
...
export default function CriticalBanner({ projectId, onOpenSource }: Props) {
  ...
  if (f.date_limite_remise.value) {
    ...
    tiles.push(
      <Tile key="remise" icon={<Calendar size={14} />} label="Remise des offres"
        accent={urgent ? '#F58E86' : undefined}>
        ...
        {days !== null && (
          <span className="ml-1.5 px-1.5 py-0.5 rounded-full text-[11px] font-bold"
            style={passed
              ? { background: 'rgba(245,142,134,0.10)', color: '#F58E86' }
              : urgent
                ? { background: 'rgba(245,142,134,0.10)', color: '#F58E86' }
  ...
  return (
    <div className="rounded-lg px-5 py-3 space-y-3" style={{ background: '#1C222D', border: '1px solid rgba(186,205,234,.13)' }}>
      {tiles.length > 0 && (
      ...
      {criteres && totalPoids > 0 && (
        <div>
          ...
          <div className="flex h-2.5 rounded-full overflow-hidden" style={{ background: '#1C222D' }}>
            {criteres.map((c, i) => (
              <div
                key={i}
                title={`${c.nom} — ${c.poids} %`}
                style={{
                  ...
                  background: /prix/i.test(c.nom) ? '#FFFFFF' : i % 2 === 0 ? '#E4E9F2' : '#9FA9BC',
                }}
              />
            ))}
          </div>
          <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1">
            {criteres.map((c, i) => (
              <span key={i} className="text-[11px]" style={{ color: '#9BA4B5' }}>
                <span className="inline-block w-2 h-2 rounded-full mr-1 align-middle"
                  style={{ background: /prix/i.test(c.nom) ? '#FFFFFF' : i % 2 === 0 ? '#E4E9F2' : '#9FA9BC' }} />
