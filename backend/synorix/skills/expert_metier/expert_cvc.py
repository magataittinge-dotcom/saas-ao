"""Skill #29 — Expert CVC (Chauffage, Ventilation, Climatisation).

Sources : NotebookLM N4 (NF DTU 65.16/68.3 + RE 2020 + F-Gaz) + N3 (Mémoires gagnants).
Raw extract: docs/notebook-extracts/skill-29-expert-cvc-raw.md
"""
...
class MethodologieCVC(BaseModel):
    phases: list[PhaseCVC]
...
class Output(SkillOutput):
    methodologie: MethodologieCVC
    sources_nbk: list[str]


@register
class ExpertCVC(Skill):
    name = "expert-cvc"
    category = "expert-metier"
    model = "claude-sonnet-4-6"
    version = "1"
    system_prompt_path = "prompts/expert_cvc.md"
...
## Produis la méthodologie CVC en JSON conforme au schéma Output.
"""
